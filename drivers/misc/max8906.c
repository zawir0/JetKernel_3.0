...
//	if (Set_MAX8906_PM_REG(WCREN, 1))
//		printk("Successfully enabled regulator WCR\n");
	/*		if (Set_MAX8906_PM_REG(WRFEN, 1))
			printk("Successfully enabled regulator WRF\n");
		if (Set_MAX8906_PM_REG(WIOEN, 1))
			printk("Successfully enabled regulator WIO\n");
		if (Set_MAX8906_PM_REG(WANAEN, 1))
			printk("Successfully enabled regulator WANA\n");
*/

//	if (Set_MAX8906_PM_REG(WCRENSRC, SW_CNTL))
//		printk("Successfully changed sequencer for WCR\n");
/*	if (Set_MAX8906_PM_REG(WRFENSRC, SW_CNTL))
		printk("Successfully changed sequencer for WRF\n");
	if (Set_MAX8906_PM_REG(WIOENSRC, SW_CNTL))
		printk("Successfully changed sequencer for WIO\n");
	if (Set_MAX8906_PM_REG(WANAENSRC, SW_CNTL))
		printk("Successfully changed sequencer for WANA\n");
*/
}
...
struct max8906_state {
	struct i2c_client	*client;
};

static int max8906_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
	...
	state = kzalloc(sizeof(struct max8906_state), GFP_KERNEL);
	if (state == NULL) {
...
//		goto error;
	...
	printk("KB: %s Assigning i2c address = 0x%x\n", __FUNCTION__, client->addr << 1);

	if ((client->addr << 1) == MAX8906_RTC_ID)
		max8906_rtc_i2c_client = client;
	else if ((client->addr << 1) == MAX8906_ADC_ID)
		max8906_adc_i2c_client = client;
	else if ((client->addr << 1) == MAX8906_GPM_ID)
...
static int max8906_remove(struct i2c_client *client)
{
	//i2c_detach_client(client);
	struct max8906_state *state = i2c_get_clientdata(client);
...
static struct i2c_device_id max8906_idtable[] = {
	{ "max8906", 0 },
	{ }
};

MODULE_DEVICE_TABLE(i2c, max8906_idtable);

static struct i2c_driver max8906_driver = {
	.driver = {
		.name = "max8906",
	},
	.id_table	= max8906_idtable,
