...
static int __init s3cfb_probe(struct platform_device *pdev)
{
...
#else
	info->clk = clk_get(NULL, "lcd_sclk");
#endif
	...
	printk("KB: %s Setting IRQs\n", __func__);

#if defined(CONFIG_CPU_S3C2443) || defined(CONFIG_CPU_S3C2450) || defined(CONFIG_CPU_S3C2416)
	ret = request_irq(IRQ_S3C2443_LCD3, s3cfb_irq, 0, "s3c-fb", pdev);

#elif defined(CONFIG_CPU_S3C6400) || defined(CONFIG_CPU_S3C6410)

	printk("KB: %s Requesting IRQ_LCD_VSYNC 0x%x\n", __func__, IRQ_LCD_VSYNC);

	ret = request_irq(IRQ_LCD_VSYNC, s3cfb_irq, 0, "s3c-fb", info);

#elif defined(CONFIG_PLAT_S5PC1XX)
	ret = request_irq(IRQ_LCD0, s3cfb_irq, 0, "s3c-fb", pdev);
